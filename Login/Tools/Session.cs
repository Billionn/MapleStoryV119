	...
	public class Session
	{
        ...
        public void Append(byte[] pBuffer, int pStart, int pLength)
        {
            ...
            if (this._type == SessionType.CLIENT_TO_SERVER)
            {
                ...
                {
                    int expr_50_cp_1 = j;
                    pBuffer[expr_50_cp_1] ^= 12;
                    pBuffer[j] = (byte)(((int)pBuffer[j] << 4 & 240) | (pBuffer[j] >> 4 & 15));
                }
            }
        ...
        private void OnInitPacketRecv(IAsyncResult ar)
        {
            ...
            if (_type == SessionType.CLIENT_TO_SERVER)
            {
                for (int i = 0; i < len; i++)
                {
                    byte[] expr_42_cp_0 = data;
                    int expr_42_cp_1 = i;
                    //Console.WriteLine("data init: " + data[i]);
                    expr_42_cp_0[expr_42_cp_1] ^= 12;
                    //Console.WriteLine("data after:  " + data[i]);
                    data[i] = (byte)(((int)data[i] << 4 & 240) | (data[i] >> 4 & 15));
                    //Console.WriteLine("data before:  " + data[i]);
                }
            }
            ...
            if (_type == SessionType.CLIENT_TO_SERVER)
            {
                Console.WriteLine("OnInitPacketReceived .");
...
//            Console.WriteLine("sendData after = " + BitConverter.ToString(input));
            SIV.Encrypt(cryptData);
//            Console.WriteLine("sendData before = " + BitConverter.ToString(cryptData));
 ...
 //       private int mSending = 0;
		...
		public void SendRawPacket(byte[] buffer)
		{
            if (_type == SessionType.SERVER_TO_CLIENT)
            {
                ...
                {
                    buffer[i] = (byte)(((int)buffer[i] << 4 & 240) | (buffer[i] >> 4 & 15));
                    int expr_2B_cp_1 = i;
                    buffer[expr_2B_cp_1] ^= 12;
                }
            }
            if (!Connected) {
                return;
